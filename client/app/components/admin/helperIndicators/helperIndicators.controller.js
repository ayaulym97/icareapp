...
class HelperStatisticsController{
	constructor($scope, StatisticService, $rootScope, $timeout, AuthorizationService) {
		"ngInject";
		...
		// Get the id of user
		this.statistic = {
			id: this.$rootScope.selectedHelper ? $rootScope.selectedHelper.id : null,
		};

		// The lists containing static information
		this.listStatistic = [
			{
		...
		// Creating current month and year
		let moment_month = moment().format("M");
		let year = moment().format("YYYY");

		// Returns the id of current month
	...
	calc_type(type){
		if(type === 0){
			return this.stats.particular.doctor_hour;
		}
		if(type === 1){
			return this.stats.particular.procedure;
		}
		if(type === 2){
			return this.stats.particular.med_test;
		}
		if(type === 3){
			return this.stats.particular.doctor_on_call;
		}
	}
	...
	// creates the start_date and end_date, then add it all to data and by using
	...
	change_date(){
		...
		let end_date = moment(new Date(this.currentYear, month + 1, 0, 0, 0, 0)).format("YYYY-MM-DD")+ " 00:00:00";
		let data = {
