...
class PersonalProfileStuffProfileController {
	$onInit() {
	...
	saveDoctor(){
		...
		console.log("[Update] Sending doctor data");
		this.HelperService.updatePersonal(data).then((response) => {
			console.log("[Update] Doctor`s data updated");
			if(this.avatar == null){
				this.$timeout(() =>{
	...
	updateDoctorInfo(data, id, ava){
		this.doctor = _find(this.PreloadService.clinicStuffList, {id : id});
		this.doctor_global = _find(this.PreloadService.helperStuffList, {id : id});
...
export default PersonalProfileStuffProfileController;
