...
class PersonalCreateProfileController {
	...
	uploadAvatar(dataUrl, name, doctor, doctorUpdateResponse) {
		...
		this.Upload.upload({
			...
			this.PreloadService.clinicDoctorList.push(doctorUpdateResponse);
			this.PreloadService.helperDoctorList.push(doctorUpdateResponse);
			this.$timeout(() => {
	...
	savePersonal() {
		...
		_forEach(this.profileData.category,(item)=> {
		  if(item.id !==0){
		    this.categoriesCreate.push(item.id);
			}
		});
		this.profileData.category = this.categoriesCreate;
		...
		this.HelperService.createPersonal(this.profileData).then((response) => {
			...
			if (this.avatar === null) {
				this.$timeout(() => {
					...
					this.PreloadService.clinicDoctorList.push(data);
					this.PreloadService.helperDoctorList.push(data);
	...
	selectCategory() {
		...
		//   name:"Нет"
		// };
		this.cpunter =_find(this.profileData.category,(item) => item.id === 0);
		if(this.cpunter) {
		...
		// this.profileData.category = this.currentCategory.id;
	}
...
export default PersonalCreateProfileController;
