...
class PersonalProfileWorktimeController {
	constructor(PreloadService,HelperService, $timeout, $rootScope, $q, $stateParams, $scope, CalendarService) {
		...
		this.doctor_global = _find(this.PreloadService.clinicDoctorList, {id : parseInt(this.$stateParams.id)});
			...
			for (let i = 8; i < 56; i++) {
				...
				if(i%4 == 3){
					...
					if(key + 1 === 24){
						...
						data["end"] = ((key) < 10 ? "0" + (key+1) : (key+1)) + ":00";
					}
				}
	...
	saveTime() {
		...
		if(removeWorktime.length > 0){
			this.HelperService.deleteWorktimeByList(removeWorktime).then((response)=>{
				this.doctor_global = _find(this.PreloadService.helperDoctorList, {id : parseInt(this.$stateParams.id)});
				this.doctor = _find(this.PreloadService.clinicDoctorList, {id : parseInt(this.$stateParams.id)});
